...
\IEEEauthorblockA{Drone Detection and Classification Engine\\
Take-Home Computer Vision Assessment}}
...
and implemented from scratch in PyTorch. The task emphasizes small-object
localization: all source images are 2560$\times$1440, contain annotated drones,
and are grouped to reduce leakage from related sequential and raw/augmented
samples. S8-CFD uses a stride-8 anchor-free prediction grid with S8/S16/S32
...
localization-focused analysis. The project constraint is also methodological:
the detector must be vanilla and implemented from scratch, without pretrained
detector weights, YOLO/Ultralytics, torchvision detector models, timm backbones,
or imported detection architectures.
...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{detector_resolution_stride_analysis.pdf}
\caption{Resolution and stride analysis used when choosing the 960$\times$544
canvas and stride-8 prediction grid for small objects.}
\label{fig:stride}
\end{figure}

\section{Experiments and Results}
The official baseline configuration is fixed for 20 epochs, batch size 2,
...
\begin{figure*}[!t]
\centering
\includegraphics[width=0.66\textwidth]{final_evaluation_qualitative_examples.pdf}
\caption{Committed representative sealed-test predictions. The 12-image subset
